...
check_items = [
    {
        ...
        "desc": "新規または変更される情報システムについて、稼働前にバリデーションを実施し、その内容を記録として保管します。TAS0313試験で利用しているシステムのCSVは実施されており、監査時に閲覧可能です。"
    },
    {
        "title": "2. 自動化システムの導入",
    ...
    {
        "title": "3. 2段階チェックとRPAの活用",
        "desc": "活動量計データ入力のような手作業の工程では、ヒューマンエラーを最小限に抑えるため、2段階のチェックとRPA（Robotic Process Automation）を導入しています。データ抽出や確認・削除も原則として担当者が一貫して行い、情報分散を防いでいます。"
    },
    {
        "title": "4. データの確認と管理",
    ...
    {
        "title": "7. BCPの策定と訓練",
        "desc": "災害時においても事業を継続するための事業継続計画（BCP）が策定されており、従業員の安否確認、全業務の複数人カバー体制、データバックアップ、クラウドシステム利用などが含まれます。BCP訓練も定期的に実施されています。"
    }
]
